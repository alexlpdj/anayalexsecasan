...
import {
    ...
    TrendingUp, CheckCircle2, XCircle, Utensils,
} from 'lucide-react';
                ...
                <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <p className={`mt-1.5 text-3xl font-bold tabular-nums ${colorText}`}>{value}</p>
                            {unit && <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-widest text-gray-400">{unit}</p>}
                            {sub && <p className="mt-1 text-xs text-gray-400">{sub}</p>}
                        </div>
                        <div className={`mt-0.5 shrink-0 rounded-xl p-2.5 ${colorBg}`}>
...
function AlertItem({ icon: Icon, iconClass, label, value, unit = 'grupos', linkHref, linkLabel, onAction, actionLabel }) {
    return (
        <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50/50 p-3">
            ...
            {linkHref && (
                <Link href={linkHref}>
                    <Button size="sm" variant="ghost" className="h-8 shrink-0 rounded-lg px-3 text-xs text-[#8b7355] hover:bg-[#8b7355]/10">
                        {linkLabel || 'Ver'} <ChevronRight className="ml-0.5 h-3 w-3" />
                    </Button>
                </Link>
            )}
            {onAction && (
                <Button size="sm" variant="ghost" className="h-8 shrink-0 rounded-lg px-3 text-xs text-[#8b7355] hover:bg-[#8b7355]/10" onClick={onAction}>
    ...
    return (
        <div className="flex items-center gap-3 py-3">
            <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
                ...
                {item.is_confirmed ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}
            </div>
            <div className="min-w-0 flex-1">
            ...
            <span className="shrink-0 text-xs tabular-nums text-gray-400">{timeAgo(item.submitted_at)}</span>
        </div>
    );
}
...
function QuestionItem({ q }) {
    return (
        <div className="flex items-start gap-3 py-3">
            ...
            <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold text-gray-500">{q.group_name}</p>
                <p className="mt-0.5 line-clamp-2 text-sm text-gray-700">{q.message}</p>
            </div>
            <span className="shrink-0 text-xs tabular-nums text-gray-400">{timeAgo(q.created_at)}</span>
        </div>
    );
}
...
export default function Dashboard({ wedding, stats, alerts, transport, recent_activity, recent_questions }) {
    ...
    const s = (i) => 0.06 + i * 0.05;

    return (
        <AdminSidebarLayout>
            <Head title="Inicio" />

            <div className="mx-auto max-w-7xl space-y-5 p-4 pb-24 sm:p-6 sm:pb-8 lg:pb-8">
                ...
                <motion.div
                    ...
                    className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#5c4433] via-[#8b7355] to-[#a89070] p-6 text-white shadow-xl sm:p-8"
                >
                    {/* Decoration */}
                    ...
                    <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div>
                            <div className="flex items-center gap-2 text-white/60">
                                ...
                                <span className="text-xs font-medium uppercase tracking-widest">Panel de boda</span>
                            </div>
                        ...
                        {countdown && !countdown.past ? (
                            <div className="flex shrink-0 gap-2 sm:gap-3">
                                {[
                                    ...
                                    <div key={l} className="flex min-w-[52px] flex-col items-center rounded-xl border border-white/20 bg-white/15 px-3 py-3 backdrop-blur-sm sm:min-w-[64px]">
                                        <span className="text-2xl font-bold tabular-nums leading-none sm:text-3xl">{pad(v)}</span>
                                        <span className="mt-1.5 text-[9px] font-semibold uppercase tracking-widest text-white/60 sm:text-[10px]">{l}</span>
                                    </div>
                                ))}
                            </div>
                        ) : countdown?.past ? (
                            <div className="rounded-xl border border-white/20 bg-white/15 px-6 py-4 text-center backdrop-blur-sm">
                ...
                <div className="grid grid-cols-3 gap-3 sm:gap-4">
                    {[
                        { label: 'Confirmados', value: stats.confirmed_groups, color: 'text-green-600', bg: 'bg-green-50',  border: 'border-green-100' },
                        { label: 'Rechazados',  value: stats.declined_groups,  color: 'text-red-500',   bg: 'bg-red-50',    border: 'border-red-100'   },
                        { label: 'Pendientes',  value: stats.pending_groups,   color: 'text-amber-600', bg: 'bg-amber-50',  border: 'border-amber-100' },
                    ].map((item, i) => (
                        <motion.div
                            key={item.label}
                            initial={{ opacity: 0, y: 14 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.35, delay: s(4 + i) }}
                        >
                            <div className={`rounded-xl border ${item.border} ${item.bg} p-3 text-center sm:p-4`}>
                                <p className={`text-2xl font-bold tabular-nums sm:text-3xl ${item.color}`}>{item.value}</p>
                                <p className="mt-0.5 text-xs font-medium text-gray-500">Grupos {item.label.toLowerCase()}</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
                ...
                <div className="grid gap-4 lg:grid-cols-5 lg:gap-5">
                    ...
                    <div className="space-y-4 lg:col-span-2">
                        ...
                        {/* Alertas */}
                        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: s(7) }}>
                            <Card className="border border-gray-100 shadow-none">
                                ...
                                <CardContent className="space-y-2 px-5 pb-5">
                                    {alerts.pending_with_email > 0 && (
                                        <AlertItem icon={Send} iconClass="bg-blue-100 text-blue-600"
                                            label="Pendientes con email" value={alerts.pending_with_email}
                                            onAction={sendReminders} actionLabel="Recordatorio" />
                                    )}
                                    {alerts.without_transport > 0 && (
                                        <AlertItem icon={Bus} iconClass="bg-orange-100 text-orange-600"
                                            label="Sin transporte confirmado" value={alerts.without_transport}
                                            linkHref={route('admin.confirmed')} linkLabel="Ver" />
                                    )}
                                    {alerts.with_allergies > 0 && (
                                        <AlertItem icon={Utensils} iconClass="bg-amber-100 text-amber-600"
                                            label="Con alergias alimentarias" value={alerts.with_allergies} unit="personas"
                                            linkHref={route('admin.confirmed')} linkLabel="Ver" />
                                    )}
                                    {!alerts.pending_with_email && !alerts.without_transport && !alerts.with_allergies && (
                                        <div className="py-6 text-center">
                                            ...
                                            <p className="mt-2 text-sm font-medium text-gray-400">Todo en orden 🎉</p>
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
                        ...
                        {/* Transporte */}
                        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: s(8) }}>
                            <Card className="border border-gray-100 shadow-none">
                                <CardHeader className="px-5 pb-2 pt-5">
                                    <CardTitle className="flex items-center gap-2 text-sm font-semibold text-gray-700">
                                        ...
                                        return (
                                            <div key={t.name}>
                                                <div className="mb-1.5 flex items-center justify-between text-xs">
                                                    ...
                                                    <span className={`font-semibold tabular-nums ${isSinDefinir ? 'text-orange-600' : 'text-gray-800'}`}>
                                                        {t.value} <span className="font-normal text-gray-400">pers.</span>
                                                    </span>
                                                </div>
                                                {!isSinDefinir && (
                        ...
                        {/* Accesos rápidos */}
                        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: s(9) }}>
                            <Card className="border border-gray-100 shadow-none">
                                <CardHeader className="px-5 pb-2 pt-5">
                                    <CardTitle className="flex items-center gap-2 text-sm font-semibold text-gray-700">
                                        Accesos directos
                                ...
                                <CardContent className="grid grid-cols-2 gap-2 px-5 pb-5">
                                    {[
                                        { label: 'Grupos',      href: route('admin.groups.index'),  icon: Users,         value: stats.total_groups    },
                                        { label: 'Confirmados', href: route('admin.confirmed'),      icon: UserCheck,     value: stats.confirmed_groups },
                                        { label: 'Preguntas',   href: route('admin.questions'),      icon: MessageCircle, value: alerts.questions_count },
                                        { label: 'Canciones',   href: route('admin.songs.index'),    icon: Music,         value: alerts.songs_count    },
                                    ].map(({ label, href, icon: Icon, value }) => (
                                        <Link key={label} href={href}>
                                            <div className="flex cursor-pointer items-center gap-2.5 rounded-xl border border-gray-100 p-3 transition-all hover:border-[#8b7355]/30 hover:bg-[#8b7355]/5">
                            ...
                            <Card className="border border-gray-100 shadow-none">
                                <CardHeader className="flex flex-row items-center justify-between px-5 pb-2 pt-5">
                                    ...
                                    <Link href={route('admin.groups.index')}>
                                        <Button variant="ghost" size="sm" className="h-7 px-3 text-xs text-[#8b7355] hover:bg-[#8b7355]/10">
                                            Ver todos <ChevronRight className="ml-0.5 h-3 w-3" />
                                        </Button>
                                    </Link>
                                </CardHeader>
                                <CardContent className="divide-y divide-gray-50 px-5 pb-2">
                                ...
                                <Card className="border border-gray-100 shadow-none">
                                    <CardHeader className="flex flex-row items-center justify-between px-5 pb-2 pt-5">
                                        ...
                                        <Link href={route('admin.questions')}>
                                            <Button variant="ghost" size="sm" className="h-7 px-3 text-xs text-violet-600 hover:bg-violet-50">
